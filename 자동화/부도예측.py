...
재무제표 = pd.read_csv('기업재무제표1.csv', encoding='cp949')

column_list = 재무제표.columns[3:]

## 사용할 재무비율 데이터 가져와서 연결재무제표 데이터가 없을 경우 개별재무제표로 채웁니다. 
...
## 사용할 회사명 코드 년도 연결 재무제표만 가져옵니다.
회사명_코드_년도 = np.array([0, 1, 2])
연결 = np.arange(3,len(재무제표.columns),2)
columns_index = np.append(회사명_코드_년도, 연결)
columns = 재무제표.iloc[:, columns_index].columns

## 열 이름이 자산(연결) 이런 식으로 되어있으므로 괄호와 괄호안의 단어를 제거한 형태로 열 이름을 바꿔줍니다.
pattern1 = r'\([^)]*\)'
columns = [re.sub(pattern1, '', s) for s in columns]
재무제표.columns = columns

## 가져온 재무비율을 가지고 사용할 재무비율 데이터로 가공합니다. 현재는 알트만의 5가지 재무비율만을 사용합니다.
...
for 회사, 결과 in zip(결과['회사명'], 결과['부도예측결과']):
    if 결과 == 1:
        print(f'{회사}는 1년이내에 부도(상장폐지)될 확률이 높다고 판단됩니다.')
    
    else:
        print(f'{회사}는 1년이내에 부도(상장폐지)될 확률이 낮다고 판단됩니다.')
